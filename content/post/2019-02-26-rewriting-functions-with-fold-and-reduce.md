...
There is a slightly difference in the Haskell `fold` and the JavaScript `reduce`, the Haskell one is curried by default but that shouldn't bother us.
...
Min is a function that gets an array and returns the minium of that array.  
The implementation is similiar to the `max`-function but with the opposite comparison.
